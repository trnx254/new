...
const scraper = async (url, comp) => {
    try {
        ...
        const title =
            $(".lBRvsb").find(".BvQan").text() ||
            $(".LDQll").find(".BvQan").text();
        const review =
        ...
        await redis.set(comp, JSON.stringify(mainData));
        await redis.expire(comp, 86400);
        return mainData;
    } catch (error) {
        console.error(error);
...
export async function GET(request, { params }) {
    ...
    const data = await scraper(url, comp);
